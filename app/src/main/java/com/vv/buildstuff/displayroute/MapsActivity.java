...
import com.vv.buildstuff.displayroute.geoFencing.ReceiveTransitionsIntentServiceOld;
...
//    private CalculateDistanceLatLng calcDistLatLng;
    ...
    /**
     * New start with geocoding
     */
    private static final float RADIUS_GEO_FENCE = 1000;
    ...
    /**
     * Location listener for direction updates
     */
    ...
    protected void onResume() {
        ...
        if (mUIGeofence1 != null) {
            mLatitude1 = mUIGeofence1.getLatitude();
            mLongitude1 = mUIGeofence1.getLongitude();
            mRadius1 = mUIGeofence1.getRadius();
        }
        if (mUIGeofence2 != null) {
            mLatitude2 = mUIGeofence2.getLatitude();
            mLongitude2 = mUIGeofence2.getLongitude();
            mRadius2 = mUIGeofence2.getRadius();
        }
    }
    ...
    public void removeGeofences(List<Geofence> geoFenceList) {
        ...
        for (Geofence geofence : geoFenceList) {
            clearGeoFences(geofence.getRequestId());
            unRegisterGeoFence(geofence.getRequestId());
        }
    }


    public void clearGeoFences(String geoFenceId) {
        mPrefs.clearGeofence(geoFenceId);
    }
    ...
    public void unRegisterGeoFence(String geoFenceId) {
        /*
     ...
     */
    public void setGeoFencing(String impGeoFenceId, LatLng impLatLng, Float impRadius, long impExpiration, int impTransition) {
             /*
        ...
        if (impExpiration == 0) {
            impExpiration = GEOFENCE_EXPIRATION_IN_MILLISECONDS;
        }

        if (impTransition == 0) {
            impTransition = Geofence.GEOFENCE_TRANSITION_ENTER | Geofence.GEOFENCE_TRANSITION_EXIT;
        }


        SimpleGeofence lclGeoFence = new SimpleGeofence(impGeoFenceId,
                impLatLng.latitude,
                impLatLng.longitude,
                impRadius,
                impExpiration,
        ...
        mPrefs.setGeofence(impGeoFenceId, lclGeoFence);
         ...
         * flat object
         */
        mCurrentGeofences.add(lclGeoFence.toGeofence());
    ...
    private void setUpListener() {
        ...
        Log.i(((Object) this).getClass().getSimpleName().toString(), "Provider Name" + provider);
...
//                        .rotation(90)
    ...
    private PendingIntent getTransitionPendingIntent() {
        Intent intent = new Intent(this, ReceiveTransitionsIntentServiceOld.class);
...
//            PolylineOptions rawLinesDir = new PolylineOptions().width(5).color(Color.rgb(28, 17, 50)).geodesic(true);
            ...
            if (null != mCurrentGeofences && mCurrentGeofences.size() > 0) {
                removeGeofences(mCurrentGeofences);
                mCurrentGeofences = new ArrayList<Geofence>();
            }
            ...
            if (latLngLists.size() >= 1) {
                ...
                int flagSetGeoFencing = 0;
                // Set the size of the geofence based on radius
                CalculateDistanceLatLng calcDistLatLng = new CalculateDistanceLatLng(RADIUS_GEO_FENCE);
                ...
                for (LatLng latLng : latLngLists.get(0)) {
                    ...
                    if (calcLatLng != null) {
                        Log.i("VV Geo Fence set ", calcLatLng.latitude + "_" + calcLatLng.longitude);
                        setGeoFencing(counter, calcLatLng, RADIUS_GEO_FENCE, GEOFENCE_EXPIRATION_IN_MILLISECONDS, Geofence.GEOFENCE_TRANSITION_ENTER | Geofence.GEOFENCE_TRANSITION_EXIT);
                        flagSetGeoFencing = 1;
                    }

                }
                if (flagSetGeoFencing == 1) {
                    addGeoFencing();
                }
